...
<body>
	<section class="header-main">
		<div class="row align-items-center">
			...
			<div class="col-lg-4">
			ÏµúÏ¥à Ï£ºÎ¨∏ : <fmt:formatDate value="${ ordersList.orders_First }" pattern="yyyy-MM-dd HH?? MMÎ∂? ssÏ¥?"/>&nbsp;/&nbsp;
			ÎßàÏ?Îß? Ï£ºÎ¨∏ : <fmt:formatDate value="${ ordersList.orders_Final }" pattern="yyyy-MM-dd HH?? MMÎ∂? ssÏ¥?"/>
			</div>
				...
				<div class="col-md-8 card padding-y-sm card ">
					...
					<div class="col-md-4">
							...
							<span id="cart">
								<table class="table table-hover shopping-cart-wrap">
									<thead class="text-muted">
										<tr>
											<th scope="col" width="150"><div align="center">Î©îÎâ¥?¥Î¶Ñ</div></th>
											<th scope="col" width="120"><div align="center">?òÎüâ</div></th>
											<th scope="col" width="120"><div align="center">?®Í?</div></th>
											<th scope="col" class="text-right" width="100"><div
													align="right">??†ú&nbsp;</div></th>
										</tr>
									</thead>
									<tbody id="orderCart">
						...
						<div class="box">
							...
							<div class="row">
								<div class="col-md-4">
									<div onclick="goPay()" class="btn  btn-primary btn-lg btn-block"><i class="fa fa-shopping-bag"></i> Í≤∞Ï†ú </div>
								</div>
								<div class="col-md-4">
									<div onclick="goDiscount()" class="btn  btn-primary btn-lg btn-block"><i class="fa fa-shopping-bag"></i> ?úÎπÑ?? </div>
								</div>
								<div class="col-md-4">
									<div onclick="removeOrderAll()" class="btn  btn-primary btn-lg btn-block"><i class="fa fa-shopping-bag"></i> ?ÑÏ≤¥Ï∑®ÏÜå </div>
								</div>
							</div>
							<br>
							<div class="row">
								<div class="col-md-4">
									<div class="btn  btn-primary btn-lg btn-block" onclick="orderStart()"><i class="fa fa-shopping-bag"></i> Ï£ºÎ¨∏ </div>
								</div>
								<div class="col-md-4">
									<div class="btn  btn-primary btn-lg btn-block" onclick="orderPrint()"><i class="fa fa-shopping-bag"></i> Ï∂úÎ†• </div>
								</div>
								<div class="col-md-4">
									<div class="btn  btn-primary btn-lg btn-block" onclick="goPosMain()"><i class="fa fa-shopping-bag"></i> Î©îÏù∏ </div>
								</div>
							</div>
						</div>
					</div>
...
function startTime() {
    ...
    var day = now.getDay();  //?îÏùº
    var hour = now.getHours();  //??
    var min = now.getMinutes();  //Î∂?
    var sec = now.getSeconds();  //Ï¥?
...
// ?´ÏûêÍ∞Ä 10Î≥¥Îã§ ?ëÏùÑ Í≤ΩÏö∞ ?ûÏóê 0?? Î∂ôÏù¥Í∏?
	...
	$.ajax({
		  ...
		  success : function(result) {
         		if(result > 0) {
         			...
         			var openWin = window.open('paymentStart.do?orders_No='+${orders_No}+price,'Í≤∞Ï†ú','width='+width+',height='+height+',status=no,scrollbars=no, left='+ popupX + ', top='+ popupY);
	          	} else {
	          		swal({
	     					title: "Ï£ºÎ¨∏?¥Ïó≠?? ?ÜÏäµ?àÎã§.",
	     					icon: "warning",
	     					button: "?´Í∏∞",
	      			});
		        }
	...
	$.ajax({
		  ...
		  success : function(result) {
       		if(result > 0) {
       			...
       			var popupX = (window.screen.width / 2) - (width / 2);
       			var popupY = (window.screen.height / 2) - (height / 2);
       			window.open('orderDiscount.do?allPrice='+$("#allPrice").html(),'?úÎπÑ??','width='+width+',height='+height+',status=no,scrollbars=no, left='+ popupX + ', top='+ popupY);
          	} else {
          		swal({
     					title: "Ï£ºÎ¨∏?¥Ïó≠?? ?ÜÏäµ?àÎã§.",
     					icon: "warning",
...
function orderStart() {
	$.ajax({
		  ...
		  success : function(result) {
           		if(result > 0) {
           			swal({
       					title: "Ï£ºÎ∞©?? Ï£ºÎ¨∏?? ?ëÏàò?òÏóà?µÎãà??.",
       					icon: "success",
       					button: "?´Í∏∞",
        			}).then((value) => {
        					var width = 1000;
        					...
        					var popupX = (window.screen.width / 2) - (width / 2);
        					var popupY = (window.screen.height / 2) - (height / 2);
        					window.open('ordersPDF.do?orders_No=${orders_No}','Ï£ºÎ¨∏?òÍ∏∞','width='+width+',height='+height+',status=no,scrollbars=yes, left='+ popupX + ', top='+ popupY);
	...
	$.ajax({
		  ...
		  success : function(result) {
           		if(result > 0) {
       				...
       				var popupX = (window.screen.width / 2) - (width / 2);
       				var popupY = (window.screen.height / 2) - (height / 2);
       				window.open('ordersPDF.do?orders_No=${orders_No}','Ï∂úÎ†•?òÍ∏∞','width='+width+',height='+height+',status=no,scrollbars=yes, left='+ popupX + ', top='+ popupY);
            	} else {
            		swal({
       					title: "Ï£ºÎ¨∏?¥Ïó≠?? ?ÜÏäµ?àÎã§.",
       					icon: "warning",
...
function onOpen(e) {
	console.log("?πÏÜåÏº? ?∞Í≤∞");	
}

function onMessage(e) {
	console.log("?úÎ≤ÑÎ°? Î∂Ä?? ?ëÎãµÎ©îÏãúÏßÄ Î∞õÏùå : " + e.data);
}

function onClose(e) {
	console.log("?πÏÜåÏª? ?´Ïùå");
}
