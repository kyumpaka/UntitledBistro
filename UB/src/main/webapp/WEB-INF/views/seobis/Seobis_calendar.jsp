   ...
   document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      dateClick: function(info) {
		...
		window.open('${path}/Seobis/Seobis_newCalendar?date='+info.dateStr ,'?àÏïΩ?òÍ∏∞','width='+width+',height='+height+',status=no,scrollbars=yes, left='+ popupX + ', top='+ popupY);
      },
      eventClick: function(info) {
          ...
          if (eventObj.url) {
        	var width = 360;
      		...
      		var popupX = (window.screen.width / 2) - (width / 2);
      		var popupY = (window.screen.height / 2) - (height / 2);
      		window.open(eventObj.url,'?àÏïΩ?ïÏù∏','width='+width+',height='+height+',status=no,scrollbars=yes, left='+ popupX + ', top='+ popupY);
